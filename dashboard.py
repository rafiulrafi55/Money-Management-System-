...
def dashboard_ui():
    ...
    def render_analytics():
        ...
        def draw_chart():
            ...
            if w < 10 or h < 10:
                # if still too small, try again after 50ms
                root.after(50, draw_chart)
                return

            amounts = [txn["amount"] for txn in sorted_txns]
            labels = [txn["datetime"].split(" ")[0] for txn in sorted_txns]  # Just date
            ...
            canvas_widget.place(x=0, y=30, width=w, height=h - 40)  # leave room for "Analytics" label

        # Delay drawing until the frame has a proper size
        root.after(50, draw_chart)
    ...
    flash_after_id = None  # global inside dashboard_ui
    flash_active = True
    ...
    def on_closing():
        ...
        if flash_after_id is not None:
            root.after_cancel(flash_after_id)  # cancel the scheduled flashing
    ...
    def update_remaining_budget():
        """
        Update the remaining budget label and trigger a warning
        if the remaining budget falls below the warning limit.
        """
        ...
        # Calculate total spent from negative transactions
        total_spent = sum(-txn["amount"] for txn in recents.values() if txn["amount"] < 0)
        remaining = budget_value - total_spent
        remaining_var.set(f"Remaining Budget {remaining:,.2f} BDT")

        # Get warning threshold from dashboard settings
        warning_limit = getattr(root, "budget_warning_limit", 500)  # default 500 if not set

        # Flashing warning if remaining < warning limit
        if remaining < warning_limit:
            budget_warning_var.set("⚠ Low budget remaining!")
            flash_warning()  # starts the flashing loop
        else:
            budget_warning_var.set("")
    ...
    transactions_change_var = tk.StringVar(value=f"+{transactions_change_value:,.2f} BDT From Previous Month")
    ...
    def budget_popup():
        ...
        def submit_budget():
            ...
            # Set new budget
            budget_value = amount
            # Remove old negative transactions
    ...
    def add_money_popup(is_add=True):
        ...
        def submit_amount():
            ...
            transactions_change_var.set(f"{sign}{abs(transactions_change_value):,.2f} BDT From Previous Month")
    ...
    def open_settings():
        """
        Modern Settings popup inside dashboard.py
        """
        popup = tk.Toplevel(root)
        ...
        # Pre-fill current warning limit
        ...
        def save_settings():
            ...
            update_remaining_budget()  # refresh warning label immediately
